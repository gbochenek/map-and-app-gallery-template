...
define([
    ...
    return declare("application.map", [Common], {
        ...
        editCommentBox: function (i) {
            var text, html, commentBody;

            // get text of comment
            text = this.globalComments[i].comment;
            ...
            commentBody = query('#comment_' + this.globalComments[i].id + ' .commentBody')[0];
            ...
            on(query('.editCommentSubmit', dom.byId("mainPanel")), "click, keyup", lang.hitch(this, function (e) {
                var comment, j;
                if (e.type === 'click' || (e.keyCode === keys.ENTER)) {
                    comment = domAttr.get(e.currentTarget, 'data-comment');
                    for (j = 0; j < this.globalComments.length; j++) {
                        if (this.globalComments[j].id === comment && this.globalItem) {
                            this.editCommentSubmit(j, comment);
                        }
                    }
                }
            }));
        ...
        /*------------------------------------*/
        locate: function () {
            var def = new Deferred(), addressQuery, queryContent;

            addressQuery = dom.byId("searchAddress").value;
            if (addressQuery && this.map) {
                queryContent = {
                    "SingleLine": addressQuery,
        ...
        createBasemapGallery: function () {
            ...
            // if node exists
            node = dom.byId("basemapContainer");
        ...
        setAddressContainer: function () {
            ...
            html += '<div class="clear"></div>';
            // Set
            node = dom.byId("addressContainer");
            this.setNodeHTML(node, html);
        },
        /*------------------------------------*/
        ...
        setGlobalItem: function (obj) {
            var def = new Deferred(), settings, q, params;
        ...
        buildComments: function () {
            var html = '', i, isOwner, commentDate, dateLocale, commentsNode;
            ...
            if (this.globalComments && this.globalComments.length > 0) {
                for (i = 0; i < this.globalComments.length; i++) {
                    isOwner = false;
                    if (this.globalUser) {
                    ...
                    // date object
                    commentDate = new Date(this.globalComments[i].created);
                    // date format for locale
                    dateLocale = locale.format(commentDate, {
                        selector: "date",
            ...
            commentsNode = dom.byId("comments");
                ...
                if (e.type === 'click' || (e.keyCode === keys.ENTER)) {
                    ...
                    for (j = 0; j < this.globalComments.length; j++) {
                        if (this.globalComments[j].id === comment && this.globalItem) {
                            ...
                            // delete comment
                            this.globalItem.deleteComment(this.globalComments[j]).then(refreshComments, refreshComments);
                        }
                    }
                }
            }));
        ...
        setRatingInfo: function () {
            var html = '', rating = '', pluralRatings, pluralComments, pluralViews, ratingNode;
        ...
        initMap: function () {
            ...
            itemDeferred.addCallback(lang.hitch(this, function (itemInfo) {
                var titleNode, subTitleNode, d, dateLocale, html = '', mapMoreInfo,
                    licenseInfo, descriptionInfo, descNode, mapDeferred, dialog;
                ...
                if (itemInfo && itemInfo.item && itemInfo.item.type === 'Web Map') {
                    ...
                    // Set title
                    titleNode = dom.byId("title");
                    this.setNodeHTML(titleNode, this._options.mapTitle);
                    // Set subtitle
                    subTitleNode = dom.byId("subtitle");
                    ...
                    html += '</ul>';
                    // set html to node
                    mapMoreInfo = dom.byId("mapMoreInfo");
                    ...
                    // Set license info
                    licenseInfo = dom.byId("licenseInfo");
                    ...
                    // Set description
                    descriptionInfo = this._options.mapItemDescription || i18n.viewer.mapPage.noDescription;
                    descNode = dom.byId("descriptionContent");
                    ...
                    // create map
                    mapDeferred = arcgisUtils.createMap(itemInfo, "map", {
                        mapOptions: {
                            slider: true,
                            sliderStyle: "small",
                            wrapAround180: true,
                    ...
                    mapDeferred.addCallback(lang.hitch(this, function (response) {
                        var layers, mapLayersNode, j, checked, k;
                        ...
                        mapLayersNode = dom.byId('mapLayers');
                        ...
                        on(query('.toggleLayers', dom.byId("sidePanel")), "click, keyup", lang.hitch(this, function (e) {
                            ...
                            if (e.type === 'click' || (e.keyCode === keys.ENTER)) {
                                dataAttr = domAttr.get(e.currentTarget, 'data-layers').split(',');
                                for (i = 0; i < dataAttr.length; i++) {
        ...
        createOptions: function () {
            var hasEsri = false, esriIdx, i, options,
        ...
        initUI: function (response) {
            var options, gc, node, layerInfo, legendDijit, legendContentNode;

            options = this.createOptions();
            gc = new Geocoder(options, "gc_search");
            gc.startup();
            // Set legend header
            node = dom.byId('legendHeader');
            ...
            // Legend Information
            layerInfo = arcgisUtils.getLegendLayers(response);
            // Build Legend
            if (layerInfo.length > 0) {
                ...
                legendContentNode = dom.byId('legendContent');
        ...
        zoomToLocation: function (x, y) {
            var lod, pt, pointSymbol, locationGraphic;

            // calculate lod
            lod = 16;
            // set point
            pt = webMercatorUtils.geographicToWebMercator(new Point(x, y));
            ...
            if (Options.pointGraphic) {
                ...
                // Create point symbol
                pointSymbol = new PictureMarkerSymbol(Options.pointGraphic, 21, 25).setOffset(0, 12);
                // Set graphic
                locationGraphic = new Graphic(pt, pointSymbol);
